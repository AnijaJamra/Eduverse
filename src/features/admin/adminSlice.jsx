...
const adminSlice = createSlice({
    ...
    extraReducers : builder => {
        ...
        .addCase(updateEvent.fulfilled, (state , action) => {
            state.adminLoading = false
            state.adminSuccess = true
            state.allEvents = state.allEvents.map(event => event._id === actionpayload._id ? action.payload : event)
...
export const updateListing = createAsyncThunk("UPDATE/LISTING/ADMIN", async (updateProduct, thunkAPI) => {
...
export const updateEvent  =  createAsyncThunk("UPDATE/EVENT/ADMIN", async(updateEvent , thunkAPI) => {
    let token = thunkAPI.getState().auth.user.token
    try {
        return adminService.update(updateEvent, token)
    } catch (error) {
        const message = error.response.data.message   
        return thunkAPI.rejectWithValue(message)
    }
