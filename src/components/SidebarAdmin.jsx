...
const SidebarAdmin = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <>
      ...
      >
        <div className="p-6">
          <h2 className="text-2xl font-bold bg-gradient-to-r from-[#fbdb86] to-[#984532] bg-clip-text text-transparent mb-6">
            Admin Panel
          </h2>
            ...
            {menuItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 ${
                  activeTab === item.id
                    ? 'bg-gradient-to-r from-[#730c16] to-[#c06970] text-white shadow-md'
